...
const grid = [
  ...
  [images.vertical, "POV: your new gym partner actually pulled up."],
  ["/gallery/lena-1.png", "Same gym. Same time. Now you know."],
  [images.wide, "Find people training when you are."],
  ...
  return (
    <div className="platformStack">
      <section className="phone light insta">
        ...
        <h3>WAITS <Verified/></h3><p className="handle">@waitsapp</p>
        <p className="bio">See who’s training—friends or someone new nearby.<br/>Post the session. Find your people. <b>Wait Up!</b></p>
      ...
      <section className="detailCard">
        <div className="detailHeader"><Avatar/><div><b>waitsapp</b> <Verified/><small>Sponsored concept</small></div><MoreHorizontal/></div>
        ...
        <p><b>3,218 likes</b></p><p><b>waitsapp</b> Your next workout partner might already train at your gym. Post your session so friends and nearby members can tap <b>Wait Up!</b></p>
        <p className="muted">View all 84 comments · 2 hours ago</p>
      </section>
      <section className="storyCard"><div className="storyImage"><img src={images.vertical}/><div className="storyTop"><Avatar size={32}/><b>waitsapp</b><span>1h</span><MoreHorizontal/></div><div className="storyCopy"><small>TONIGHT · 7:30 PM</small><h2>Chest at 7?</h2><p>Mazzi + 3 friends are going.</p><button>WAIT UP!</button></div></div><p><b>Story concept:</b> one-tap urgency with a real workout invite.</p></section>
    </div>
  );
}
...
const tiktoks = [
  ...
  ["/gallery/mike-1.png", "Meet your next gym partner", "12.8K"],
] as const;

function TikTokPreview() {
  return <div className="platformStack"><section className="phone dark tiktok"><div className="phoneTop"><strong>9:41</strong><span>● ◒ ▰</span></div><div className="appBar"><ArrowLeft/><strong>Profile</strong><Menu/></div><div className="center"><Avatar size={90}/><h2>WAITS <Verified/></h2><p>@waitsapp</p><div className="metrics compact"><Metric value="214" label="Following"/><Metric value="18.6K" label="Followers"/><Metric value="246.4K" label="Likes"/></div><button className="follow">Follow</button><p>Find friends and new workout partners nearby. 🏋️‍♀️</p></div><div className="videoGrid">{tiktoks.map(([src,text,views])=><div key={text}><img src={src}/><b>{text}</b><span><Play/> {views}</span></div>)}</div></section><section className="tiktokFeature"><img src={images.vertical}/><div className="shade"/><div className="ttCopy"><b>@waitsapp</b><p>POV: you posted “chest at 7?” and someone at your gym tapped Wait Up! #gymtok #workoutpartner</p><small>♫ original sound — WAITS</small></div><div className="rail"><Avatar/><Heart/><b>48.2K</b><MessageCircle/><b>613</b><Bookmark/><b>4,190</b><Share2/><b>1,204</b></div></section></div>;
}

function Tweet({ pinned, text, stats }: { pinned?: boolean; text: string; stats: string[] }) {
  return <article className="tweet">{pinned && <small className="pinned">📌 Pinned</small>}<div className="tweetBody"><Avatar/><div><b>WAITS <Verified/></b> <span className="muted">@waitsapp · 2h</span><p>{text}</p><div className="tweetStats"><span><MessageCircle/> {stats[0]}</span><span><Repeat2/> {stats[1]}</span><span><Heart/> {stats[2]}</span><span><Share2/></span></div></div></div></article>;
}

function XPreview() { return <section className="desktopMock xMock"><div className="xBanner"><img src={images.wide}/><div className="bannerWords"><Brand inverse/><h2>Never lift alone.</h2><p>Post the workout. Find out who’s in.</p></div></div><div className="xProfile"><Avatar size={96}/><button>Follow</button><h2>WAITS <Verified/></h2><p className="muted">@waitsapp</p><p>Post workouts, join friends, or meet someone new who trains nearby. Find the right workout partner before the session starts.</p><p className="muted">📍 Wherever you train · 🔗 waits.app</p><div className="followStats"><b>186</b> Following&nbsp;&nbsp; <b>12.8K</b> Followers</div></div><Tweet pinned text="Introducing WAITS: post when and where you’re training, then let friends and nearby gym members tap Wait Up! Never lift alone." stats={["92","418","2.1K"]}/><Tweet text="Chest at 7?" stats={["164","91","1.8K"]}/><Tweet text="Your next workout partner might already train at your gym." stats={["47","602","3.4K"]}/><Tweet text="Same gym. Same schedule. Now you can actually find each other." stats={["31","284","1.6K"]}/></section>; }

function FacebookPreview() { return <section className="desktopMock fbMock"><div className="fbCover"><img src={images.wide}/><div><h1>Who’s waiting up?</h1><p>Turn plans into workouts.</p></div></div><div className="fbIdentity"><Avatar size={104}/><div><h2>WAITS <Verified/></h2><p>App · Community · Fitness</p><b>8.7K followers</b></div><button>Follow</button></div><div className="fbTabs"><b>Posts</b><span>About</span><span>Photos</span><span>Videos</span><MoreHorizontal/></div><div className="fbColumns"><aside><h3>About</h3><p>WAITS helps people coordinate workouts with friends and people nearby.</p><p>🏋️ Built for people who already have a gym membership.</p><p>📱 Post. Join. Train together.</p></aside><main><article className="fbPost"><div className="detailHeader"><Avatar/><div><b>WAITS <Verified/></b><small>2h · 🌐</small></div><MoreHorizontal/></div><p><b>“I didn’t know y’all were going.”</b><br/>That sentence is officially retired.</p><img src={images.square}/><div className="engagement">💙 ❤️ 1.2K <span>86 comments · 41 shares</span></div><div className="postActions"><b>Like</b><b>Comment</b><b>Share</b></div><div className="comment"><img src="/avatars/sarah-model.jpg"/><p><b>Sarah Kim</b><br/>Finally. Our group chat is chaos 😂</p></div></article><article className="fbPost"><div className="detailHeader"><Avatar/><div><b>WAITS</b><small>Yesterday · 🌐</small></div></div><p>Tonight: legs at 6:30. Tomorrow: pretending stairs don’t exist. Who’s in?</p></article></main></div></section>; }
...
const threadPosts = [
  ...
  ["Your future gym partner could be one public workout post away.", "2,744", "143"],
...
function ThreadsPreview() { return <section className="phone light threads"><div className="phoneTop"><strong>9:41</strong><span>● ◒ ▰</span></div><div className="threadsTitle"><b>WAITS</b><span>@</span></div><div className="threadsProfile"><div><h2>WAITS <Verified/></h2><p>waitsapp</p></div><Avatar size={72}/><p>Find friends and new workout partners nearby.<br/>Post it. See who taps Wait Up! 🏋️</p><p><span className="stackedAvatars"><img src="/avatars/sarah-model.jpg"/><img src="/avatars/danny-model.jpg"/></span> 9.4K followers</p><button>Follow</button></div>{threadPosts.map(([text,likes,replies],i)=><article className="thread" key={text}><Avatar size={38}/><div><b>waitsapp <Verified/></b><span className="muted"> {i+1}h</span><p>{text}</p>{i===0&&<div className="reply"><img src="/avatars/danny-model.jpg"/><p><b>dannylifts</b><br/>Wait up. I’m in 🤝</p></div>}<div className="threadActions"><Heart/><MessageCircle/><Repeat2/><Send/></div><small>{replies} replies · {likes} likes</small></div><MoreHorizontal/></article>)}<div className="bottomNav"><Home/><Search/><Plus/><Heart/><Avatar size={25}/></div></section>; }
...
const strategy: Record<Platform, { hook: string; goal: string; format: string }> = {
  ...
  TikTok:{hook:"POV: you found someone who trains when you do.",goal:"Earn shares and app curiosity",format:"Fast, human, trend-aware video"},
  ...
  Threads:{hook:"Start conversations between people who love training.",goal:"Create community voice",format:"Conversation-first posts"},
};

export function SocialPreviewGallery() {
  const [active, setActive] = useState<Platform>("Instagram");
  const Preview = previews[active];
  return <main className="galleryPage"><header className="galleryHero"><div><Brand inverse/><p>WAITS SOCIAL LAUNCH CONCEPTS</p><h1>Make the invite feel impossible to ignore.</h1><p className="lede">Five launch-ready ways to show how WAITS connects friends and new workout partners around real gym plans.</p></div><ConceptBadge/></header><nav className="platformTabs" aria-label="Social platform previews">{platforms.map(p=><button key={p} onClick={()=>setActive(p)} className={active===p?"active":""}>{p}</button>)}</nav><div className="galleryLayout"><aside className="strategy"><span className="eyebrow">{active.toUpperCase()} DIRECTION</span><h2>{strategy[active].hook}</h2><div><small>PRIMARY GOAL</small><b>{strategy[active].goal}</b></div><div><small>BEST FORMAT</small><b>{strategy[active].format}</b></div><div><small>BRAND VOICE</small><b>Friendly. Direct. A little playful.</b></div><blockquote>“Find out who’s training—and never lift alone.”</blockquote><p className="disclaimer">These visuals, people, account details, and engagement figures are fictional concepts for internal brand planning.</p></aside><div className="previewStage"><div className="stageLabel"><span><span className="liveDot"/> LIVE CONCEPT VIEW</span><span>Switch tabs to compare</span></div><Preview/></div></div><footer><Brand inverse/><p>Mockups only · No social accounts or integrations connected.</p></footer><style jsx global>{styles}</style></main>;
}
