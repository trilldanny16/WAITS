  ...
  return (
    <div className="platformStack">
      ...
      <section className="detailCard">
        ...
        <p><b>3,218 likes</b></p><p><b>waitsapp</b> Your boys were at the same gym again—you just found out from the post-workout selfie. Post your session. Let the crew tap <b>Wait Up!</b></p>
...
function TikTokPreview() {
  return <div className="platformStack"><section className="phone dark tiktok"><div className="phoneTop"><strong>9:41</strong><span>● ◒ ▰</span></div><div className="appBar"><ArrowLeft/><strong>Profile</strong><Menu/></div><div className="center"><Avatar size={90}/><h2>WAITS <Verified/></h2><p>@waitsapp</p><div className="metrics compact"><Metric value="214" label="Following"/><Metric value="18.6K" label="Followers"/><Metric value="246.4K" label="Likes"/></div><button className="follow">Follow</button><p>Your workout group chat, actually organized. 🏋️‍♀️</p></div><div className="videoGrid">{tiktoks.map(([src,text,views])=><div key={text}><img src={src}/><b>{text}</b><span><Play/> {views}</span></div>)}</div></section><section className="tiktokFeature"><img src={images.vertical}/><div className="shade"/><div className="ttCopy"><b>@waitsapp</b><p>POV: you posted “chest at 7?” and the whole crew actually pulled up. #gymtok #workoutpartner</p><small>♫ original sound — WAITS</small></div><div className="rail"><Avatar/><Heart/><b>48.2K</b><MessageCircle/><b>613</b><Bookmark/><b>4,190</b><Share2/><b>1,204</b></div></section></div>;
}
...
function XPreview() { return <section className="desktopMock xMock"><div className="xBanner"><img src={images.wide}/><div className="bannerWords"><Brand inverse/><h2>Never lift alone.</h2><p>Post the workout. Find out who’s in.</p></div></div><div className="xProfile"><Avatar size={96}/><button>Follow</button><h2>WAITS <Verified/></h2><p className="muted">@waitsapp</p><p>Your workout group chat, actually organized. Post workouts, join friends, and stop finding out afterward that everybody trained without you.</p><p className="muted">📍 Anywhere your crew trains · 🔗 waits.app</p><div className="followStats"><b>186</b> Following&nbsp;&nbsp; <b>12.8K</b> Followers</div></div><Tweet pinned text="Introducing WAITS: the fastest way to turn ‘we should hit the gym sometime’ into an actual workout. Post the time. Your friends tap Wait Up! That’s it." stats={["92","418","2.1K"]}/><Tweet text="Chest at 7?" stats={["164","91","1.8K"]}/><Tweet text="Stop finding out AFTER the workout that your boys were at the same gym." stats={["47","602","3.4K"]}/><Tweet text="Your Tuesday workout shouldn’t need 46 unread group-chat messages to happen." stats={["31","284","1.6K"]}/></section>; }
