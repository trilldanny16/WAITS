  ...
  return (
    <div>
      <button
        ...
        disabled={sharing}
        className="flex h-12 w-full items-center justify-center gap-2 rounded-2xl bg-accent text-sm font-bold text-accent-foreground shadow transition-colors hover:brightness-95 focus-visible:outline-2 focus-visible:outline-primary disabled:opacity-60"
      >
        <Share2 size={18} aria-hidden="true" />
        {sharing ? 'Opening Share…' : 'Share WAITS'}
      </button>
      <p role="status" aria-live="polite" className="mt-1 text-center text-xs text-muted-foreground">
        {message}
      </p>
      {showLink ? (
        <input
